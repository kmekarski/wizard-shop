...
*/
  ...
  const usersHtml = (
    props.type === "pending"
      ? orders.filter(
          (order) => order.orderState === 0 || order.orderState === 1
        )
      : props.type === "completed"
      ? orders.filter((order) => order.orderState === 2)
      : orders.filter((order) => order.orderState === 3)
  ).map((el, index) => {
    let date = new Date(el.dateCreated);
    return (
      <div
        className={`orders-list__row ${
          index % 2 === 1 ? "orders-list__row--gray" : ""
        }`}
      >
        <p className="text--small-regular text--dark">{el.id}</p>
        <p className="text--small-regular text--dark">{el.email}</p>
        <p className="text--small-reguler text--dark">
      ...
      <div className="container">
        ...
        <div className="orders-list__main">
          <div className="orders-list__card card--big">
            {headerHtml}
            <div className="orders-list__card__rows">{usersHtml}</div>
          </div>
        </div>
      </div>
    </div>
